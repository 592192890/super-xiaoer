<template>
	<div>
		...
		<section class="ui-table">
			<el-table :data="formEntity" v-loading="listLoading" highlight-current-row ref="multipleTable" style="width: 100%;">
				<el-table-column prop="id" label="序号" show-overflow-tooltip>
				</el-table-column>
				<el-table-column prop="shopName" label="店铺名称" show-overflow-tooltip>
				</el-table-column>
				<el-table-column prop="recordCreateTime" label="申请入驻时间" show-overflow-tooltip>
				</el-table-column>
				<el-table-column prop="companyAddress" label="所在地" show-overflow-tooltip>
				</el-table-column>
				...
				<el-table-column label="操作">
					<template scope="scope">
						...
						<span class="audit-btn" v-if="scope.row.status=='01'&&buttonArr.indexOf('bt-audit')!=-1" @click="handleAudit(scope.$index, scope.row)">审核</span>
					</template>
				</el-table-column>
			</el-table>
			<!--工具条-->
...
export default {
	...
	data(){
		...
		return {
			titles:[{id:1,name:'店铺中心'},{id:2,name:'招商审核'}],
			buttonArr:[],
			currentPage:1,
			listLoading: true,
			total:0,
	...
	methods:{
		search:function(index){
			...
			var url=this.$apiUrl.getInvestListUrl;
        ...
        handleAudit:function(index,row){
			this.$router.push({name:'审核招商信息',params:{shopId:row.id,currentPage:this.currentPage}})
        },
		handleCheck:function(index,row){
			this.$router.push({name:'查看招商信息',params:{shopId:row.id,currentPage:this.currentPage}})
	...
	mounted(){
		...
		//下拉框
		if(!sessionStorage.getItem('investAuditStatus')){
			var url=this.$apiUrl.getInvestStatusUrl;
			this.$ajax.get(url).then(res=>{
				if(res.error==0){
					this.statusList=res.data||[];
					sessionStorage.setItem('investAuditStatus',JSON.stringify(res.data))
				}
				
			})
		}else{
			this.statusList=JSON.parse(sessionStorage.getItem('investAuditStatus'));
		}
	}
}
</script>
