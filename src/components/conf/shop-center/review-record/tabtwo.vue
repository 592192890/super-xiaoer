<template>
	<section class="ui-table tabtwo">
		...
		<el-table :data="model" v-loading="listLoading" highlight-current-row ref="multipleTable" style="width: 100%;" >
			<el-table-column prop="cateName" label="分类名称" show-overflow-tooltip>
			</el-table-column>
			<el-table-column prop="afterSecurityFee" label="最后一次保证金修改事件" show-overflow-tooltip >
			</el-table-column>
			<el-table-column prop="afterPercentage" label="最后一次分佣比例修改事件" show-overflow-tooltip>
			</el-table-column>
			...
			this.$ajax.get(url, param).then(res=>{
				...
				if(res.data.list && res.data.list.length){
					res.data.list.forEach(val=>{
						val.afterSecurityFee = val.applyId+"从"+val.beforeSecurityFee+"元修改为"+val.afterSecurityFee+"元";
						val.afterPercentage = val.applyId+"从"+val.beforePercentage+"%修改为"+val.afterPercentage+"%";
