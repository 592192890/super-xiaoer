		...
		<section>
			<el-col :span="24" class="mod-ctn">
				<el-form ref="formEntity" :rules="rules" :model="formEntity" label-position="right" label-width="120px">
                    ...
                    <el-form-item label="上级店铺分类：">
                        <el-col :span="6" >{{ cateName }}</el-col>
                    </el-form-item>
...
export default {
    ...
    data(){
        return{
            titles:[{id:1,name:'店铺中心'},{id:2,name:'店铺分类管理'},{id:3,name:'添加店铺分类'}],
            cateName:"",
            id:"",
            parentId:"",
            formEntity: {
    ...
    methods:{
        submitForm:function(formName){           
            ...
            this.$refs[formName].validate(valid => {
                if (valid) { 
                    this.$ajax.get(url,{name:cateName, parentId:this.id, sort:cateSort}).then(res =>{
                        if(res.error==1){
                            return
                        }
                        ...
                        this.$router.push({name:'店铺分类管理',query: { id: id, parentId:this.id, mode:'display'}})
                    })
                } else {
