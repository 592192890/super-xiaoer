    ...
    <div class="classify-add">
        ...
        <section class="right-ctn">
            <div class="mod-steps">
                ...
                <div class="step-three">3，添加成功</div>
            </div>
            <el-col :span="24" class="mod-toolbar">
                <el-form ref="ruleForm" :model="formEntity" v-loading="loading" label-width="150px" label-position="left" class="form">
                    ...
                    <el-form-item label="按商品关键词筛选">
                        ...
                        <el-col :span="12">
                            <span class="text-link">商品搜索关键词【商品名称】或【商品编码】，多个词以【,】分隔开</span>
                        </el-col>
                    </el-form-item>
...
export default {
    ...
    data(){
        return{
            options:[],
            pageSize: 7,
            titles:[{id:1,name:'店铺中心'},{id:2,name:'前台展示管理-新增'}],
            countries: [],
            errorMsg1:'',
            errorMsg2:'',
            formEntity:{
    ...
    methods:{
        ...
        submitForm:function(){     
            ...
            let url = this.$apiUrl.addFrontClassifyUrl;
            ...
            param.parentId = this.parentId || 0
            ...
            this.$ajax.post(url, param).then(res =>{
                ...
                if(res.error) return
                let item = res.data
                this.$router.push({ name: '前台展示管理-新增完成', query: { catId: item.catId, parentId:item.parentId,catName:item.catName, imageKey:item.imageKey,linkUrl:item.linkUrl, targetName:item.targetName, targetType:item.targetType,imageUrl:this.imageUrl}})
            }) 
        },

        goBack(){
            this.$router.push({ name: '前台展示管理-新增1', query:{targetName: this.targetName, parentId:this.parentId, addLevelOne:this.addLevelOne, catName:this.catName,imageKey:this.imageKey,linkUrl:this.linkUrl,targetType:this.targetType,imageUrl:this.imageUrl}});
        }
