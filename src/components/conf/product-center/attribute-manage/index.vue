  ...
  <div>
    ...
    <section style="margin-top:20px">
      <el-row :span="24" class="mod-toolbar" :gutter="10">
        <el-col :span="6">
          <el-button type="primary" class="add-btn" @click="add">新增属性</el-button>
        ...
        <el-col :span="18">
          <div class="g-fr">
            ...
            <el-col :span="6">
              <el-input v-model="searchEntity.typeName" @keyup.enter.native="search()" placeholder="请输入属性名称"></el-input>
            </el-col>
            <el-col :span="2">
    ...
    <section class="ui-table">
      <el-table :data="formEntity" highlight-current-row
        ...
        <el-table-column prop="typeName" label="属性名称"></el-table-column>
        <el-table-column label="操作" width="150">
          <template scope="scope">
            <span class="edit-btn cursor" @click="handleEdit(scope.$index, scope.row)" v-if="buttonArr.indexOf('bt-edit')!=-1">编辑</span>
            <span class="del-btn cursor" @click="handleDel(scope.$index, scope.row)" v-if="buttonArr.indexOf('bt-delete')!=-1">删除</span>
          </template>
        </el-table-column>
      </el-table>
      <!--工具条-->
...
<script>
  ...
  export default{
    ...
    data(){
      return {
        titles: [{id: 1, name: '商品中心'}, {id: 2, name: '属性管理'}],
    ...
    methods: {
      ...
      search: function (index = 1) {
        ...
        let param = {
          currentPage: index,
          pageSize: 20,
          typeName: this.searchEntity.typeName
        }
      ...
      deleteAction: function (ids = []) {
        const url = this.$apiUrl.delAttributeUrl
        const param = {
...
//        deleteRow = row
      ...
      handleEdit: function (index, row) {
        this.$router.push({name: '属性编辑', params: {typeId: row.typeId, currentPage: this.currentPage}})
      },
      /**
       * 表格内选择项发生变化事件
