<template>  
    <div class="sku">
       ...
       <el-form :rules="rules" ref="ruleForm" :model="formEntity" label-width="80px" style="margin-top:20px">
            <section style="margin-top:20px">
                <div class="basic-info mod-toolbar">  
                    ...
                    <el-row :gutter="40">
                        ...
                        <el-col :span="20">
                            <div class="label item">{{formEntity.cateStr}}</div>
                    ...
                    <el-row :gutter="40">
                        ...
                        <el-col :span="10">
                            <el-form-item prop="spuName">
                                <el-input v-model="formEntity.spuName" placeholder="长度至少为2个字符，最长50个字符" :disabled="isFromRetek"></el-input>
                            </el-form-item>
                        </el-col>
                    </el-row>
                    <el-row :gutter="40">
                    ...
                    <el-row :gutter="40">
                        ...
                        <el-col :span="10">
                            <el-form-item prop="spuBn">
                                <el-input v-model="formEntity.spuBn" @change="changeSpuBn" placeholder="2-50个字符，仅支持数字，字母" :disabled="isFromRetek" v-bind:class="{error:customErrorSpuBn}"></el-input>
                    ...
                    <el-row :gutter="40" v-if="isFromRetek">
                        <el-col :span="4">
                            <div class="label require-label">品牌</div>
                        </el-col>
                        <el-col :span="6">
                            <el-form-item>
                                <el-select v-model="formEntity.brandId" disabled filterable placeholder="请选择">
                    ...
                    <el-row :gutter="40">
                        ...
                        <el-col :span="6">
                            <el-form-item prop="catchWeightInd">
                                <el-select v-model="formEntity.catchWeightInd" placeholder="请选择" @change="changeIsWeight" :disabled="isFromRetek">
                    ...
                    <el-row v-for="item in specArr" :key="item.id">
                     ...
                     <el-row v-if="!selectNoSpecRadio&&isHasSpec">
                        <el-col :span="6" :offset="4">
                            <el-button class="spec-btn" :disabled="isFromRetek" @click="generateSpec">生成规格</el-button>
                        </el-col>
                        ...
                        <el-col :span="24">
                            <table class="custom-table">
                                ...
                                <tr v-for="(item,index) in specEntity" :key="item.specName">
                                    ...
                                    <td>
                                        <div class="cell" :span="2">
                                            <el-input v-model="item.skuName" @change="validateSpec(item,'skuName')" v-bind:class="{error:item.isError_skuName}"></el-input>
                                            
                                        </div>
                                        <div :span="2" v-if="item.isError_skuName" class="validate-error">
                                            <span>2-50个字符</span>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="cell" :span="2">
                                            <el-input v-model="item.skuBn" @change="validateSpec(item,'skuBn',index)" v-bind:class="{error:item.isError_skuBn||item.isExist_skuBn||item.isDulplicate_skuBn}" :disabled="isFromRetek"></el-input>
                                        </div>
                                        <div :span="2" v-if="item.isError_skuBn" class="validate-error">
                                            <span>2-50个字符</span>
                                        </div>
                                    ...
                                    <td>
                                        <div class="cell" :span="2">
                                            <el-input v-model="item.weight" @change="validateSpec(item,'weight')" v-bind:class="{error:item.isError_weight}" :disabled="isFromRetek"></el-input>
                                        </div>
                                    ...
                                    <td>
                                        <div class="cell" :span="2">
                                            <el-input v-model="item.mktPrice" @change="validateSpec(item,'mktPrice')" v-bind:class="{error:item.isError_mktPrice}" :disabled="isFromRetek"></el-input>
                                        </div>
                                    ...
                                    <td>
                                        <div class="cell" :span="2">
                                            <el-input v-model="item.barCode" @change="validateSpec(item,'barCode',index)" v-bind:class="{error:item.isError_barCode||item.isExist_barCode||item.isDulplicate_barCode}" :disabled="isFromRetek"></el-input>
                                        </div>
                                        <div :span="3" v-if="item.isError_barCode" class="validate-error">
                                            <span>6-50个数字字母</span>
                                        </div>
                                    ...
                                    <td v-if="isFromRetek">
                                        <div class="cell" :span="2">
                                            <el-input v-model="item.cost" @change="validateSpec(item,'cost')" v-bind:class="{error:item.isError_cost}" :disabled="isFromRetek"></el-input>
                                        </div>
                                        <div :span="3" v-if="item.isError_cost" class="validate-error">
                                            <span>6-50个数字字母</span>
                                        </div>
                                    </td>
                                </tr>
            ...
            <section>
                <div class="pic-info mod-toolbar">
                    ...
                    <el-row v-if="!isClickGenerateNoSpecButton" v-for="(item,index) in specPicArr" :key="item.skuName">
                        <el-col :span="4">
...
export default {
    ...
    data(){
        ...
        return {
            ...
            titles:[{id:1,name:'商品中心'},{id:2,name:'SPU管理'},{id:3,name:'SPU编辑'}],
            judgePicType:'',//判断是商品图片还是规格图片；'product'是商品
            isFromRetek:false,//判断是否来源于spu模板，如果是的话，一下字段不能编辑
            isHasSpec:false,//有无规格radio
      
            selectedFirstCategory:{},
            selectedSecondCategory:{},
            selectedThirdCategory:{},
            specEntity:[],
            backupSpecEntity:[],//备份specEntity去判断输入的货号条形码
            customErrorField:{
    ...
    methods:{
                ...
                if(type=='weight'){
                    ...
                    if(/^[\d\w]{2,50}$/.test(item[type])){
                        ...
                        if(index!=undefined){
                            ...
                            bnTimer=setTimeout(()=>{
                                this.isExistDataBase('skuBn',item[type],item,index)
                            },300)
                        }
        ...
        generateSpec:function(){
            ...
            var nameObj={};
            var tempNameArr=[];
            newSpecNameArr.forEach(v=>{
                if(!nameObj[v]){
                    nameObj[v]=1;
                    tempNameArr.push(v)
                }
            })
            newSpecNameArr=tempNameArr;
            //整理后的规格信息id
            var newSpecIdArr=this.doExchange(specIdArr);
            //去重
            var idObj={};
            var tempIdArr=[];
            newSpecIdArr.forEach(v=>{
                if(!idObj[v]){
                    idObj[v]=1
                    tempIdArr.push(v)
                }
            })
            newSpecIdArr=tempIdArr;
        ...
        goSave:function(formName){
            ...
            this.$refs[formName].validate((valid) => {     
                if (valid) {
                    ...
                    //判断是否是无规格，如果是无规格需要将商品图片加到sku数组里面去
                    if(this.isClickGenerateNoSpecButton){
                        //无规格
                        var tempIds=[];
                        this.productPicArr.forEach(v=>{
                    ...
                    this.$ajax.post(url,params).then(res=>{
                        if(res.error==0){
                            ...
                            this.$router.push({name:'SPU管理',params:{currentPage:sessionStorage.getItem('spuCurrentPage')}})
                        }else{
                            if(res.msg=="spu编号重复"){
                                this.customErrorSpuBn=true;
                            }
        ...
        getSPUDeatil:function(spuId){
            var url=this.$apiUrl.getSKUDetailUrl+"?id="+spuId;
            ...
            this.$ajax.get(url).then(res=>{
                if(res.error==0){
                    ...
                    //是否有规格
                    // this.isHasSpec=res.data.spu.hasSpecs==0?false:true;
                    

                    //是否显示规格sku数据
                    if(res.data.skuResultList&&res.data.skuResultList.length){
                        this.specEntity=res.data.skuResultList.map(v=>{
                            v=Object.assign(v,this.customErrorField);
                            
                            return v;
                        });
                        this.backupSpecEntity=JSON.parse(JSON.stringify(this.specEntity));
                        ...
                        if(!res.data.skuResultList[0].specText||res.data.skuResultList[0].specText.length==0){
                            ...
                            //生成specName
                            this.specEntity[0].specName='无规格';
                    ...
                    //自定义扩展属性
                    var propsAttributes=res.data.spu.propsAttributes;
                    ...
                    //非自定义扩展属性
                    var propsText=res.data.spu.propsText;
        ...
        querySpecNameBySpecId:function(parentId,specId){
            var specName="";
            this.specArr.forEach(v=>{
                if(v.id==parentId){
                    v.values.forEach(vv=>{
    ...
    mounted(){
        if(this.$route.params.entity){
            sessionStorage.setItem('spuItem',JSON.stringify(this.$route.params.entity));
        }
        var spuItem=JSON.parse(sessionStorage.getItem('spuItem'));
        var spuId=spuItem.spuId;
        

        this.getSPUDeatil(spuId);
