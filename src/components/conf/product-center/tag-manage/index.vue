  ...
  <div>
    ...
    <section style="margin-top:20px">
      <el-row :span="24" class="mod-toolbar" :gutter="10">
        <el-col :span="6">
          <el-button type="primary" class="add-btn" @click="add">新增标签</el-button>
        ...
        <el-col :span="18">
          <div class="g-fr">
            ...
            <el-col :span="6">
              <el-input v-model="searchEntity.labelName" @keyup.enter.native="search()" placeholder="请输入标签名称"></el-input>
            </el-col>
            <el-col :span="2">
    ...
    <section class="ui-table">
      <el-table :data="formEntity" highlight-current-row
        ...
        <el-table-column prop="startDate" label="生效开始时间">
          <template scope="scope">
            <label v-html="formatDate(scope.row.startDate)"></label>
          </template>
        </el-table-column>
        <el-table-column prop="endDate" label="生效结束时间">
          <template scope="scope">
            <label v-html="formatDate(scope.row.endDate)"></label>
          </template>
        </el-table-column>
        <el-table-column prop="isShow" label="显示到前台">
          <template scope="scope">
            <label v-if="scope.row.isShow === 1">显示</label>
            <label v-else>不显示</label>
          </template>
        </el-table-column>
        <!--<el-table-column prop="disabled" label="卖家是否可用">
          <template scope="scope">
            <label v-if="scope.row.disabled">可用</label>
            <label v-else>不可用</label>
          </template>
        </el-table-column>-->
...
//  let deleteRow = null;
  export default{
    ...
    data(){
      return {
        titles: [{id: 1, name: '商品中心'}, {id: 2, name: '标签管理'}],
    ...
    methods: {
      ...
      search: function (index = 1) {
        let url = this.$apiUrl.getTagListUrl
        let param = {
          pageNum: index,
          pageSize: this.pageSize,
          labelName: this.searchEntity.labelName
        }
      ...
      deleteAction: function (ids = []) {
        const url = this.$apiUrl.deleteTagUrl
        const param = {
...
//        this.deleteAction([deleteRow.labelId])
//        this.deleteDialogVisible = false
      ...
      handleEdit: function (index, row) {
        this.$router.push({name: '标签编辑', params: {labelId: row.labelId, currentPage: this.currentPage}})
      },
      /**
       * 表格内选择项发生变化事件
