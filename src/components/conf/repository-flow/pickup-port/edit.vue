      ...
      <el-col :span="24" class="mod-toolbar">
        <el-form :rules="rules" ref="ruleForm" :model="formEntity" label-width="120px" label-position="left" class="form">
          ...
          <el-form-item label="自提点地址" required>
            <el-col :span="6">
              <cascader-select type="edit" :circleItem="formEntity.area" v-on:returnCascaderArr="getCascaderArr" v-if="isShowCascaderSelect"></cascader-select>
...
<script>
  ...
  export default {
    ...
    data(){
      ...
      return {
        titles: [{id: 1, name: '仓库物流'}, {id: 2, name: '自提点管理'}, {id: 3, name: '编辑'}],
        pickupAddressList:[],
    ...
    methods: {
      ...
      update: function (formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            var url = this.$apiUrl.updatePickUpUrl
      ...
      getDetail(id){
        var url=this.$apiUrl.getPickUpDetailUrl;
        var params={
          id:id
        }
        this.$ajax.get(url,params).then(res=>{
          if(res.error==0){
            ...
            //得到areaName
            var result=this.formEntity.area;
            this.formEntity.areaName=result.province.name+result.city.name+result.district.name;
    ...
    mounted(){
      if(this.$route.params.id){
        sessionStorage.setItem('pickup-itemId',this.$route.params.id)
      }

      var id=sessionStorage.getItem('pickup-itemId');
