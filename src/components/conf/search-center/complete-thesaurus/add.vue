...
export default {
    ...
    data(){
        return{
            titles:[{id:1,name:'搜索中心'},{id:2,name:'补全词库'},{id:3,name:'新增'}],
			formInline:{
				value:"",
			},
			rules:{
	...
	methods: {
		submitForm(formName) {
            this.$refs[formName].validate((valid) => {
                if (valid) {
					this.$ajax.post(this.$apiUrl.complementedAdd,{keyword:this.formInline.value}).then((res) =>
					{
						this.$message({
						message: "‘"+this.formInline.value+"’"+"词名新增成功！",
						type: 'success'
						});
						this.$router.push({name:'补全词库'});
					});
                } else {
                    return false;
                }
