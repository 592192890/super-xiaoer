	...
	<div class="page-view">
		...
		<el-form class="form" :inline="true" :model="formInline">
			...
			<el-form-item label="查询" class="search-form">
				<el-input class="search-input" v-model="formInline.value" placeholder="请输入同义词"></el-input>
		...
		<div class="ui-table">
			<el-table :data="tableData" style="width: 100%" v-loading="listLoading" @selection-change="selectionChange">
				...
				<el-table-column label="词名" prop="wordGroup"></el-table-column>
...
export default {
    ...
    data(){
        ...
        return{
			param,
            titles:[{id:1,name:'搜索中心'},{id:2,name:'同义词库'}],
			formInline:{
				value:"",
				fileName:''
			},
	...
	methods: {
        onSearch() {
			var self = this;
			this.param.key = this.formInline.value;
			self.getData();
		},
		onReset(){
			this.param.key = "";
		...
		getData()
		{
			...
			self.$ajax.get(self.$apiUrl.synonymList,self.param).then((res) =>
			{
		...
		del(){
			if (this.selectionId.length) {
				this.$confirm('您确定要删除吗？', '提示', {
					...
					this.$ajax.post(this.$apiUrl.synonymDel,{ids:selectionId}).then((res) =>
					{
						this.$message({
						message: "删除成功！",
						type: 'success'
