	...
	<div class="page-view">
		...
		<el-form class="form" :inline="true" :model="formInline">
			...
			<el-form-item label="查询" class="search-form">
				<el-input class="search-input" v-model="formInline.value" placeholder="请输入分词"></el-input>
		...
		<div class="ui-table">
			<el-table :data="tableData" style="width: 100%" v-loading="listLoading" @selection-change="selectionChange">
				...
				<el-table-column label="词名" prop="word"></el-table-column>
        ...
        :before-close="handleClose">
			<el-form :rules="rules" ref="ruleForm" :model="formInline">
				<el-form-item prop="fileName"  
					...
					class="upload-demo"
					ref="upload"
					:action="$apiUrl.participleImport"
...
export default {
    ...
    data(){
        ...
        return{
			param,
            titles:[{id:1,name:'搜索中心'},{id:2,name:'分词词库'}],
			formInline:{
				value:"",
				fileName:''
			},
	...
	methods: {
       onSearch() {
		...
		getData()
		{
			...
			self.$ajax.get(self.$apiUrl.participleList,self.param).then((res) =>
			{
		...
		editor(index)
		{
			this.$router.push({name:'编辑分词'});
		},
		add(){
			this.$router.push({name:'新增分词'});
		},
		del(){
			if (this.selectionId.length) {
				this.$confirm('您确定要删除吗？', '提示', {
					...
					this.$ajax.post(this.$apiUrl.participleDel,{ids:selectionId}).then((res) =>
					{
						this.$message({
						message: "删除成功！",
						type: 'success'
